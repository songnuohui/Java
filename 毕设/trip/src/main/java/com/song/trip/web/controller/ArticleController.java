...
public class ArticleController {
	...
	public String saveFoodWords(Words words){
		if(words != null){
			String food_id = words.getLw_food_id();
			foodService.saveWords(words);
			return "redirect:toFoodArticleView.do?food_id="+food_id;
		}else{
			return null;
		}
	}
	 ...
	 * 保存回复信息
	 */
	@RequestMapping(value="/saveFoodReply")
	public String saveFoodReply(Reply reply){
		if(reply != null){
			foodService.saveReply(reply);
			String food_id = reply.getLr_food_id();
			return "redirect:toFoodArticleView.do?food_id="+food_id;
		}else{
			return null;
		}
	...
	public String toFoodArticleView(@RequestParam int food_id, Model model){
		//封装留言信息
		lw_list = foodService.findByWords();
		model.addAttribute("lw_list",lw_list);

		//封装回复信息
		lr_list = foodService.findByReply();
		model.addAttribute("lr_list",lr_list);
		...
		System.out.println("查询到当前文章的ID值：" + food.getTpFid());
