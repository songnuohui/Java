...
import com.song.trip.web.entity.FoodShopCart;
import com.song.trip.web.entity.HotelShopCart;
import com.song.trip.web.entity.ViewShopCart;
...
public class UserController {
	...
	public String userLogin(String uemail, String upwd, Model model, HttpServletRequest request) {
		//index页面登录成功图片显示路径
		...
		if (user != null) {
			 ...
			 * 登录即创建景点购物车在session中
			 * **/
			ViewShopCart shopCart=new ViewShopCart();
			 ...
			 * 登录即创建酒店购物车在session中
			 * **/
			HotelShopCart hotelShopCart=new HotelShopCart();
			HttpSession session1 = request.getSession();
			session1.setAttribute("hotelShopCart",hotelShopCart);

			/**
			 * 登录即创建美食购物车在session中
			 * **/
			FoodShopCart foodShopCart=new FoodShopCart();
			HttpSession session2=request.getSession();
			session2.setAttribute("foodShopCart",foodShopCart);

			this.session.setAttribute("user", user);
			return "redirect:/index";
		} else {
			this.session.setAttribute("msg", Msg.fail("还未注册或邮箱密码错误,请重新输入!"));
			return "redirect:/index";
		}
	...
	public String forum(Integer uid, Model model) {
		...
		for(Forum forum : forums){
			if (uid.equals(Integer.parseInt(forum.getTpAuthorId()))){
